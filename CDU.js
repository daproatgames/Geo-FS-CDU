...
const pad3 = (n) => String(Math.round(Number(n) || 0)).padStart(3, "0");
...
// -------------------- GeoFS accessors --------------------
const geoReady = () => !!(window.geofs?.animation?.values && window.geofs?.autopilot?.values);
...
if (!geoReady()) return { ok: false, why: "geofs not ready" };
...
const hdgN = hdg !== undefined ? toFinite(hdg) : null;
const spdN = spd !== undefined ? toFinite(spd) : null;
const altN = alt !== undefined ? toFinite(alt) : null;
const vsN = vs !== undefined ? toFinite(vs) : null;
...
if (hdgN !== null) {
try {
geofs.autopilot.values.course = norm360(hdgN);
wrote++;
} catch {}
}

// SPEED / ALT often treated as strings internally
if (spdN !== null) {
try { geofs.autopilot.values.speed = String(Math.max(0, Math.round(spdN))); wrote++; } catch {}
}
if (altN !== null) {
try { geofs.autopilot.values.altitude = String(Math.max(0, Math.round(altN))); wrote++; } catch {}
}
if (vsN !== null) {
try { geofs.autopilot.values.verticalSpeed = Math.round(vsN); wrote++; } catch {}
}

return wrote ? { ok: true, wrote } : { ok: false, why: "no writes applied" };
}
...
// keyboard input (only when CDU is visible)
document.addEventListener("keydown", (e) => {
if (!root || root.style.display === "none") return;

// allow F9 to toggle even when visible
if (e.key === "F9") {
...
html += `<div class="dim">Press L1..L6 to DIR. Sets HDG target. EXEC applies.</div>`;
...
const lv = LV();
const ap = APV();
const apCourse = Number(geofs.autopilot?.values?.course ?? ap.course ?? 0);
...
html += line("AP HDG", `<span class="mag">${pad3(apCourse)}°</span>`);
html += line("AP SPD", `<span class="mag">${String(ap.speed ?? geofs.autopilot?.values?.speed ?? "")}</span>`);
html += line("AP ALT", `<span class="mag">${String(ap.altitude ?? geofs.autopilot?.values?.altitude ?? "")}</span>`);
}
...
const ap = APV();
const apCourse = Number(geofs.autopilot?.values?.course ?? ap.course ?? 0);
html += line("HDG SEL", `<span class="white">${pad3(apCourse)}°</span>`);
html += line("SPD SEL", `<span class="white">${String(ap.speed ?? geofs.autopilot?.values?.speed ?? "")}</span>`);
html += line("ALT SEL", `<span class="white">${String(ap.altitude ?? geofs.autopilot?.values?.altitude ?? "")} FT</span>`);
html += line("VS SEL", `<span class="white">${String(ap.verticalSpeed ?? geofs.autopilot?.values?.verticalSpeed ?? 0)} FPM</span>`);
...
if (label === "HIDE") { if (root) root.style.display = "none"; return; }
...
cduPad.className = "control-pad geofs-radio-pad blue-pad"; // matches style vibe
cduPad.tabIndex = 0;

// Keep it consistent with other pads
cduPad.innerHTML = `<div class="control-pad-label transp-pad">CDU</div>`;
cduPad.title = "Open CDU";

cduPad.onclick = () => toggleCDU();
...
const t = setInterval(() => {
addCduPadBelowRadio();
if (++tries > 60 || document.getElementById("geofs-cdu-pad")) clearInterval(t);
}, 250);
}
